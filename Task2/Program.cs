...
namespace Task2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<TransformationStrategy> transformationLists = new List<TransformationStrategy>();
            transformationLists.Add(new FileTransformation(root));
            transformationLists.Add(new FileContentTransformation());

            while (dfs.HasNext())
            {
                var node = dfs.doNext();
                IFileSystemNode transformedNode = null;
                foreach (var transformation in transformationLists)
                {
                    if(transformedNode == null)
                    {
                        transformedNode = transformation.transform(node);
                    } else
                    {
                    transformedNode = transformation.transform(transformedNode);
                    }
                }

                Console.WriteLine(transformedNode.GetPrintableName());
                var printableContent = transformedNode.GetPrintableContent();
                if (printableContent != null)
                {
                    Console.WriteLine(printableContent);
                }
